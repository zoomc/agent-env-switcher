...
Manages Hermes AI coding assistant profiles.
Works with the same profile storage as the Agent Env Switcher GUI.

Commands:
  list              List all Hermes profiles
  ...
  apply <id>        Apply profile to ~/.hermes/config.yaml
...
PROFILES_FILE = STORAGE_DIR / "hermes-profiles.json"
...
def cmd_list():
    ...
    if not profiles:
        ...
        print("Use 'hermes-profile.py add' to create one.")
        return
...
def cmd_add():
    profile_id = f"hermes-{int(__import__('time').time() * 1000)}"
...
def cmd_apply(profile_id: str):
    ...
    config_content = f"""# Generated by Agent Env Switcher
...
"""
    ...
    if CONFIG_PATH.exists():
        backup_path = CONFIG_PATH.with_suffix(f".yaml.bak.{int(__import__('time').time())}")
        CONFIG_PATH.rename(backup_path)
        print(f"Backup created: {backup_path}")

    CONFIG_PATH.write_text(config_content)

    profile["lastApplied"] = __import__("datetime").datetime.now().isoformat()
    save_profiles(profiles)
...
def main():
    ...
    if command == "list":
        ...
        if len(sys.argv) < 3:
            print("Usage: hermes-profile.py show <profile-id>", file=sys.stderr)
            sys.exit(1)
        cmd_show(sys.argv[2])
    elif command == "switch":
        if len(sys.argv) < 3:
            print("Usage: hermes-profile.py switch <profile-id>", file=sys.stderr)
            sys.exit(1)
        ...
        if len(sys.argv) < 3:
            print("Usage: hermes-profile.py update <profile-id>", file=sys.stderr)
            sys.exit(1)
        cmd_update(sys.argv[2])
    elif command == "delete":
        if len(sys.argv) < 3:
            print("Usage: hermes-profile.py delete <profile-id>", file=sys.stderr)
            sys.exit(1)
        cmd_delete(sys.argv[2])
    elif command == "apply":
        if len(sys.argv) < 3:
            print("Usage: hermes-profile.py apply <profile-id>", file=sys.stderr)
            sys.exit(1)
