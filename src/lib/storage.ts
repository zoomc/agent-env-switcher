...
export function validateImport(jsonString: string): ImportValidation {
  ...
  for (let i = 0; i < profiles.length; i++) {
    ...
    if (typeof p.apiKey !== 'string') {
      return {
        valid: false,
        error: `Profile "${p.name}" has an invalid 'apiKey' (must be a string)`,
      };
    }
    ...
    if (typeof p.defaultModel !== 'string') {
      return {
        valid: false,
        error: `Profile "${p.name}" has an invalid 'defaultModel' (must be a string)`,
      };
    }
    ...
    if (typeof p.fastModel !== 'string') {
      return {
        valid: false,
        error: `Profile "${p.name}" has an invalid 'fastModel' (must be a string)`,
      };
    }
    ...
    if (typeof p.reasoningModel !== 'string') {
      return {
        valid: false,
        error: `Profile "${p.name}" has an invalid 'reasoningModel' (must be a string)`,
      };
    }
    ...
    if (!Array.isArray(p.enabledTargets)) {
      return {
        valid: false,
        error: `Profile "${p.name}" has an invalid 'enabledTargets' (must be an array)`,
      };
    }
